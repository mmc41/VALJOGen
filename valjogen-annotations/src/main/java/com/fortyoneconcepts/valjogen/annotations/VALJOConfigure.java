...
public @interface VALJOConfigure
{
	...
	*
	* Each entry is of form <code>&lt;method name&gt; "(" &lt;unqualifed parameter type name&gt; { "," &lt;unqualifed parameter type name&gt; } ")" "=" &lt;annotation string&gt;</code>. For example to annotate a method "f" taking an integer and a String
	* as arguments write <code>f(int,String)=@MyAnnotation(...)</code>. Note that all type names are unqualified. To specify a constructor set &lt;method_name&gt; to an empty string. To specify a factory method set method name to "valueOf".
	*
